...
$(() => {
    $('#table-data').on('click', '.btn-delete', function () {
        ...
        Swal.fire({
            title: 'Anda yakin?',
            html: `Anda akan menghapus prodi "<b>${prodi}</b>"!`,
            ...
            if (result.isConfirmed) {
                $.post(BASE_URL + 'program-studi/delete', {
                    id,
                    _method: 'DELETE'
                }).done((res) => {
    ...
    $('#form-prodi-update').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            beforeSend: () => {
                clearErrorMessage();
                $('#modal-prodi-update').find('.modal-dialog').LoadingOverlay('show');
            },
            success: (res) => {
                $('#modal-prodi-update').find('.modal-dialog').LoadingOverlay('hide', true);
                $(this)[0].reset();
                clearErrorMessage();
                table.ajax.reload();
                $('#modal-prodi-update').modal('hide');
            },
    ...
    function load_edit_form(id_ubah) {
        ...
        $.ajax({
            url: BASE_URL + 'program-studi/show_edit_form',
            type: 'get',
            data: {
                id_ubah: id_ubah,
            },
            success: function(response) {
                window.location.href = BASE_URL + 'program-studi/show_edit_form?id_ubah=' + id_ubah;
    ...
    table = $('#table-data').DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + 'program-studi/data',
            type: 'get',
            dataType: 'json'
        },
        order: [[1, 'desc']],
        ...
        columns: [{
            ...
            data: 'fakultas',
            render: function(data, type, row) {
                // Check if 'fakultas' data is available
                if (data && data.length > 0) {
                    // Map the 'fakultas' array to get an array of fakultas names
                    var fakultasNames = data.map(function(fakultas) {
                        return fakultas.fakultas;
                    });
                    // Join the fakultas names using a delimiter (comma, for example)
                    return fakultasNames.join(', ');
                } else {
                    // If 'kampus' data is not available, return an empty string
                    return '';
                }
